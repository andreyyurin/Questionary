...
class FloatingActionsMenu : ViewGroup {
    ...
    private fun init(context: Context, attributeSet: AttributeSet?) {
        mButtonSpacing =
            (resources.getDimension(R.dimen.fab_actions_spacing) - resources.getDimension(R.dimen.fab_shadow_radius) - resources.getDimension(
        ...
        mAddButtonPlusColor = attr.getColor(
            R.styleable.FloatingActionsMenu_fab_addButtonPlusIconColor, getColor(
                android.R.color.white
            )
        )
        mAddButtonColorNormal = attr.getColor(
            R.styleable.FloatingActionsMenu_fab_addButtonColorNormal, getColor(
                android.R.color.holo_blue_dark
            )
        )
        mAddButtonColorPressed = attr.getColor(
            R.styleable.FloatingActionsMenu_fab_addButtonColorPressed, getColor(
    ...
    private fun createAddButton(context: Context) {
        mAddButton = object : AddFloatingActionButton(context) {
            ...
            override val iconDraw: Drawable
                get() {
                    val rotatingDrawable = RotatingDrawable(super.iconDraw)
        ...
        mAddButton!!.setOnClickListener(OnClickListener { toggle() })
        addView(mAddButton, super.generateDefaultLayoutParams())
        mButtonsCount++
    }

    fun setIconToMainButton(@DrawableRes icon: Int) {
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        when (mExpandDirection) {
            EXPAND_UP, EXPAND_DOWN -> {
                ...
                val addButtonY = if (expandUp) b - t - mAddButton!!.getMeasuredHeight() else 0
                ...
                val addButtonLeft: Int =
                    buttonsHorizontalCenter - mAddButton!!.getMeasuredWidth() / 2
                mAddButton?.layout(
                    addButtonLeft,
                    addButtonY,
                ...
                var nextY =
                    if (expandUp) addButtonY - mButtonSpacing else addButtonY + mAddButton!!.getMeasuredHeight() + mButtonSpacing
                var i = mButtonsCount - 1
                while (i >= 0) {
                    val child = getChildAt(i)
                    if (child === mAddButton || child.visibility == GONE) {
                        i--
                        continue
                    }
            ...
            EXPAND_LEFT, EXPAND_RIGHT -> {
                ...
                val addButtonX = if (expandLeft) r - l - mAddButton!!.getMeasuredWidth() else 0
                // Ensure mAddButton is centered on the line where the buttons should be
                val addButtonTop: Int =
                ...
                var nextX =
                    if (expandLeft) addButtonX - mButtonSpacing else addButtonX + mAddButton!!.getMeasuredWidth() + mButtonSpacing
                var i = mButtonsCount - 1
                while (i >= 0) {
                    val child = getChildAt(i)
                    if (child === mAddButton || child.visibility == GONE) {
                    ...
                    val childY: Int =
                        addButtonTop + (mAddButton!!.getMeasuredHeight() - child.measuredHeight) / 2
                    child.layout(
                        childX,
                        childY,
                        childX + child.measuredWidth,
    ...
    private fun createLabels() {
        val context: Context = ContextThemeWrapper(context, mLabelsStyle)
        for (i in 0 until mButtonsCount) {
            val button: FloatingActionButton = getChildAt(i) as FloatingActionButton
            val title: String = button.title
